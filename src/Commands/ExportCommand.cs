...
{
return @"Usage: binance-p2p-monitor export [options]
...
binance-p2p-monitor export --output=prices.csv.gz --compress
";
}
...
var compress = context.HasOption("compress");
...
{
// Handle compression
var compressedPath = outputPath;
// If output path doesn't end with .gz, add it
if (!compressedPath.EndsWith(".gz", StringComparison.OrdinalIgnoreCase))
{
compressedPath += ".gz";
}

_output.WriteInfo($"Compressing output to {compressedPath}...");
...
_output.WriteSuccess($"Data exported successfully to {Path.GetFullPath(compressedPath)}");
}
else
{
// Regular uncompressed export
